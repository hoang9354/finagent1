...
from modules.linear_dependence import run_linear_dependence_tests
...
# ─── Phase 2b — Linear Dependence Tests ──────────────────────────────────────

def phase_linear_dependence(cleaned: dict, run_id: str = ""):
    _banner("Phase 2b — Linear Dependence Tests")
    t0 = time.time()

    out_dir = os.path.join(cfg.REPORTS_DIR, run_id) if run_id else cfg.REPORTS_DIR
    results = run_linear_dependence_tests(
        cleaned=cleaned,
        output_dir=out_dir,
        run_id=run_id,
        lb_lags=10,
    ...
    n_autocorr  = sum(1 for v in results.get("autocorrelation",   {}).values() if v.get("reject_h0"))
    n_granger   = sum(1 for v in results.get("granger_causality", {}).values() if v.get("reject_h0"))
    n_coint     = sum(1 for v in results.get("cointegration",     {}).values() if v.get("reject_h0"))

    _check(f"Autocorrelation   : {n_autocorr}/{len(cleaned)} assets show serial dependence")
    _check(f"Granger causality : {n_granger} significant directed pairs")
    _check(f"Cointegration     : {n_coint} cointegrated price pairs")
    _check(f"Results saved     → {out_dir}")
    print(f"\n  Tests complete in {time.time() - t0:.1f}s")
    return results
...
def main():
    ...
    phase_linear_dependence(cleaned, run_id=run_id)
